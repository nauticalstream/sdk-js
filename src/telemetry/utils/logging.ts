...
import { peekCorrelationId, getTraceId, getSpanId } from './context.js';
...
export function createLogger(options: TelemetryLoggerOptions = {}, destination?: DestinationStream): TelemetryLogger {
  ...
  const userMixin = pinoOptions.mixin as ((mergeObject: object, level: number) => object) | undefined;
  ...
  const baseOptions: LoggerOptions = {
    ...
    mixin(mergeObject: object, level: number) {
      ...
      const telemetryFields: Record<string, string> = {};
      if (correlationId) telemetryFields.correlationId = correlationId;
      if (traceId)       telemetryFields.traceId       = traceId;
      if (spanId)        telemetryFields.spanId        = spanId;
      ...
      logMethod(inputArgs, method, level) {
        ...
        if (level >= minLevel) {
          if (SentryModule) {
            ...
            let logContext: unknown = firstArg;

            if (firstArg instanceof Error) {
              capturedError = firstArg;
            } else if (typeof firstArg === 'object' && firstArg !== null) {
              const raw = (firstArg as any).error ?? (firstArg as any).err;
            ...
            SentryModule.withScope((scope) => {
              if (logContext !== undefined) scope.setContext('log', logContext as Record<string, unknown>);
